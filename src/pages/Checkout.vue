    ...
    <form class="checkout__form" @submit.prevent="submitHandler" novalidate>
      <div class="checkout__form__input">
        ...
        <section>
            ...
            <div class="methods">
              ...
              >
                <input
                  ...
                  v-model="picked"
                />
                <label for="card" class="radio-label">Card Payment</label>
              </div>
              <div
                :class="[
                  'radio-container',
          ...
          <div class="checkout__form__input__item" v-if="picked === 'card'">
            <div class="input-texts">
              <label for="card-number" class="no-capitalize"
                >Card Number</label
              >
              <p
                class="empty-message"
                v-if="emptyFields.includes('card-number')"
              >
                Field can't be empty
              </p>
            </div>
            ...
            <input
              ...
              ref="cardNumber"
              :class="
                emptyFields.includes('card-number') ? 'empty-border' : ''
              "
              @click="wipeError('card-number')"
              @change="wipeError('card-number')"
            />
          </div>
          <div
          ...
          >
            <div class="input-texts">
              <label for="card-pin" class="no-capitalize">card PIN</label>
              <p
                class="empty-message"
                v-if="emptyFields.includes('card-pin')"
              >
                Field can't be empty
              </p>
            </div>
            ...
            <input
              ...
              id="card-pin"
              ref="cardPin"
              :class="emptyFields.includes('card-pin') ? 'empty-border' : ''"
              @click="wipeError('card-pin')"
              @change="wipeError('card-pin')"
            />
          </div>
      ...
      <div class="checkout__form__summary">
        ...
        <input
          type="submit"
          :value="picked === 'card' ? 'Continue & pay' : 'Continue'"
...
export default {
  ...
  methods: {
    ...
    submitHandler() {
      ...
      const cardRefs = [this.$refs.cardNumber, this.$refs.cardPin];
      myRefs.map((ref) => {
        if (ref.value === "") {
      ...
      if (this.picked === "cash") {
        if (this.emptyFields.includes("card-number")) {
          this.wipeError("card-number");
        }
        if (this.emptyFields.includes("card-pin")) {
          this.wipeError("card-pin");
        }
      }
      this.validateEmail();
  ...
  &__form {
    ...
    &__input {
      ...
      .cash-info {
        ...
        &__icon {
          font-size: 3rem;
   color: rgb(196, 1, 1);          margin-right: 3.2rem;
        }
